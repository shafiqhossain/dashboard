...
class ViewTotalExpiringRxForm extends FormBase {
  ...
  */
  ...
  public function getFormID() {
    return 'custom_example_total_expiring_rx_view_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    // Rx pending
    $rx_expiring_nids = $this->database->select('dashboard_count_by_date', 'n')
      ->condition('n.dashboard_date', date('Y-m-d'))
      ->fields('n', ['rx_expiring_nids'])
      ->execute()
      ->fetchField();
    ...
    if (!empty($rx_expiring_nids)) {
      $rows = explode(',', $rx_expiring_nids);
      if (count($rows) > 0) {
        foreach ($rows as $row) {
          $links = explode('|', $row);
          $nid = (isset($links[0]) && !empty($links[0]) ? $links[0] : 0);
          $amount = (isset($links[1]) && !empty($links[1]) ? number_format($links[1],2) : '');
          if ($nid) {
