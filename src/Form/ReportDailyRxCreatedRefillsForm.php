...
class ReportDailyRxCreatedRefillsForm extends FormBase {
  ...
  */
  ...
  public function getFormID() {
    return 'custom_example_report_daily_rx_created_refills_form';
  }
   ...
   */
  public function buildForm(array $form, FormStateInterface $form_state, $rx_base_type = NULL, $action_type = NULL) {
    if (empty($rx_base_type)) {
      throw new AccessDeniedHttpException('Sorry! No valid Rx type reference found.');
    }
    ...
    if ($rx_base_type == 'slit') {
      if ($action_type == 'created') {
        ...
        $query->innerJoin('node__field_rx_amount', 'ra', "ra.entity_id = n.nid and bt.bundle = 'rx'");
        $query->where("DATE_FORMAT(od.field_order_date_value, '%Y-%m-%d') = :order_date_value", [':order_date_value' => date("Y-m-d")], '=');
        $query->condition('ws.field_workflow_status_value', 'Cancelled', '!=');
        $query->condition('n.type', 'rx');
        $query->condition('n.status', 1);
        $query->condition('bt.field_rx_base_type_value', 'SLIT');
    ...
    elseif ($rx_base_type == 'scit') {
      if ($action_type == 'created') {
        ...
        $query->condition('n.type', 'rx');
        $query->condition('n.status', 1);
        $query->condition('bt.field_rx_base_type_value', 'SCIT');
        $query->fields('n', ['nid', 'title']);
        $query->addExpression('od.field_order_date_value', 'rx_date');
        $query->addExpression('ra.field_rx_amount_value', 'amount');
      }
    ...
    if (count($results) > 0) {
      ...
      $rx_links .= '  <tr>';
      $rx_links .= '    <td colspan="3">Sorry! No rx information found!</td>';
      $rx_links .= '  </tr>';
    }
    $rx_links .= '</tbody>';
    $rx_links .= '</table>';
