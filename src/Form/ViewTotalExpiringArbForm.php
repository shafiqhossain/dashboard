...
class ViewTotalExpiringArbForm extends FormBase {
  ...
  */
  ...
  public function getFormID() {
    return 'custom_example_total_expiring_arb_view_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    // Arb expiring
    $arb_expiring_ids = $this->database->select('dashboard_count_by_date', 'n')
      ->condition('n.dashboard_date', date('Y-m-d'))
      ->fields('n', ['arb_expiring_ids'])
      ->execute()
      ->fetchField();

    $arb_links = '<table cellspacing="0" cellpadding="5" class="arb-link-list">';
    $arb_links .= '<thead>';
    $arb_links .= '  <tr>';
    $arb_links .= '    <th>Subscription ID</th>';
    $arb_links .= '    <th>Expire Date</th>';
    $arb_links .= '  </tr>';
    $arb_links .= '</thead>';
    $arb_links .= '<tbody>';

    if (!empty($arb_expiring_ids)) {
      $rows = explode(',', $arb_expiring_ids);
      if (count($rows)>0) {
        foreach ($rows as $row) {
          $links = explode('|', $row);
          ...
          $arb_links .= '  <tr>';
          $arb_links .= '    <td>' . $subscription_id . '</td>';
          $arb_links .= '    <td>' . $expire_date . '</td>';
          $arb_links .= '  </tr>';
        }
      }
    }
      ...
      $arb_links .= '  <tr>';
      $arb_links .= '    <td colspan="2">Sorry! No information found!</td>';
      $arb_links .= '  </tr>';
    }
    $arb_links .= '</tbody>';
    $arb_links .= '</table>';
    ...
    $form['results'] = [
      '#markup' => Markup::create($arb_links),
