...
class ViewTotalRxScheduledForm extends FormBase {
  ...
  */
  ...
  public function getFormID() {
    return 'custom_example_total_rx_scheduled_view_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    // Rx scheduled
    $rx_scheduled_nids = $this->database->select('dashboard_count_by_date', 'n')
      ->condition('n.dashboard_date', date('Y-m-d'))
      ->fields('n', ['rx_scheduled_nids'])
      ->execute()
      ->fetchField();
    ...
    $rx_links .= '  <tr>';
    $rx_links .= '    <th>Rx</th>';
    $rx_links .= '    <th>Total Amount</th>';
    $rx_links .= '  </tr>';
    $rx_links .= '</thead>';
    $rx_links .= '<tbody>';

    if (!empty($rx_scheduled_nids)) {
      $rows = explode(',', $rx_scheduled_nids);
      if (count($rows)>0) {
        foreach ($rows as $row) {
          $links = explode('|', $row);
          ...
          $amount = $node->hasField('field_rx_amount') && !$node->get('field_rx_amount')->isEmpty() ?
            number_format($node->get('field_rx_amount')->value,2) : '';
      ...
      $rx_links .= '  <tr>';
      $rx_links .= '    <td colspan="2">Sorry! No information found!</td>';
      $rx_links .= '  </tr>';
    }
