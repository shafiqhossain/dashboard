...
class ViewTotalExpiringCreditCardsForm extends FormBase {
  ...
  */
  ...
  public function getFormID() {
    return 'custom_example_total_expiring_cc_view_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    // Rx pending
    $cc_expiring_ids = $this->database->select('dashboard_count_by_date', 'n')
      ->condition('n.dashboard_date', date('Y-m-d'))
      ->fields('n', ['cc_expiring_ids'])
      ->execute()
      ->fetchField();

    $cc_links = '<table cellspacing="0" cellpadding="5" class="cc-link-list">';
    $cc_links .= '<thead>';
    $cc_links .= '  <tr>';
    $cc_links .= '    <th>Subscription ID</th>';
    $cc_links .= '    <th>Notification Date</th>';
    $cc_links .= '  </tr>';
    $cc_links .= '</thead>';
    $cc_links .= '<tbody>';

    if (!empty($cc_expiring_ids)) {
      $rows = explode(',', $cc_expiring_ids);
      if (count($rows)>0) {
        foreach ($rows as $row) {
          $links = explode('|', $row);
          $subscription_id = (isset($links[0]) && !empty($links[0]) ? $links[0] : 0);
          $expire_date = (isset($links[1]) && !empty($links[1]) ? date('Y-m-d', strtotime($links[1])) : '');

          $cc_links .= '  <tr>';
          $cc_links .= '    <td>' . $subscription_id . '</td>';
          $cc_links .= '    <td>' . $expire_date . '</td>';
          $cc_links .= '  </tr>';
        }
      }
    }
      ...
      $cc_links .= '  <tr>';
      $cc_links .= '    <td colspan="2">Sorry! No information found!</td>';
      $cc_links .= '  </tr>';
    }
    $cc_links .= '</tbody>';
    $cc_links .= '</table>';
