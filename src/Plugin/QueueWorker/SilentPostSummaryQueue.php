...
/**
 * Defines 'custom_example_silent_post_summary_queue' queue worker.
 *
 * @QueueWorker(
 *   id = "custom_example_silent_post_summary_queue",
 *   title = @Translation("Silent Post Summary Queue"),
 *   cron = {"time" = 60},
 * )
 */
class SilentPostSummaryQueue extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public function processItem($data) {
    $this->logger->info('custom_example_silent_post_summary_worker run completed @ ' . date('Y-m-d H:i:s'));

    $data = $this->dashboardManager->getSilentPostSummaryByYear(date("Y"));
    $total_resolved = !empty($data['total_resolved']) ? $data['total_resolved'] : 0;
    $total_pending = !empty($data['total_pending']) ? $data['total_pending'] : 0;

    // Update the table
    $this->dashboardManager->setBillingCountByDate(
      [
        'dashboard_date' => date('Y-m-d'),
        'silent_post_resolved' => $total_resolved,
        'silent_post_pending' => $total_pending,
        'silent_post_last_update' => date('Y-m-d H:i:s'),
      ],
      [
        'silent_post_resolved' => $total_resolved,
        'silent_post_pending' => $total_pending,
