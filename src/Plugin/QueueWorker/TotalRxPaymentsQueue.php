...
/**
 * Defines 'custom_example_total_rx_payments_queue' queue worker.
 *
 * @QueueWorker(
 *   id = "custom_example_total_rx_payments_queue",
 *   title = @Translation("Total Rx Payments Queue"),
 *   cron = {"time" = 60},
 * )
 */
class TotalRxPaymentsQueue extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public function processItem($data) {
    $this->logger->info('custom_example_total_rx_payments_worker run completed @ ' . date('Y-m-d H:i:s'));
    ...
    // Invoice payment
    $total_invoice_payment = $this->dashboardManager->getRxInvoicePaymentByDate(date("Y-m-d"));
    ...
    // This week
    $data = $this->dashboardManager->getRxPaymentsByWeek(date("Y"), date("W"));
    $total_denied_payment = !empty($data['total_denied_payment']) ? $data['total_denied_payment'] : 0;
    $total_successful_payment = !empty($data['total_successful_payment']) ? $data['total_successful_payment'] : 0;
    $total_refund_payment = !empty($data['total_refund_payment']) ? $data['total_refund_payment'] : 0;
    $total_void_payment = !empty($data['total_void_payment']) ? $data['total_void_payment'] : 0;
    ...
    // Invoice payment
    $total_invoice_payment = $this->dashboardManager->getRxInvoicePaymentByWeek(date("Y"), date("W"));
    ...
    // Invoice payment
    $total_invoice_payment = $this->dashboardManager->getRxInvoicePaymentByWeek(date("Y"), date("n"));
