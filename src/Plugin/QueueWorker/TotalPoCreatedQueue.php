...
/**
 * Defines 'custom_example_total_po_created_queue' queue worker.
 *
 * @QueueWorker(
 *   id = "custom_example_total_po_created_queue",
 *   title = @Translation("Total PO Created Queue"),
 *   cron = {"time" = 60},
 * )
 */
class TotalPoCreatedQueue extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public function processItem($data) {
    $this->logger->info('custom_example_total_po_created_worker run completed @ ' . date('Y-m-d H:i:s'));
    ...
    // This week
    $data = $this->dashboardManager->getPoCreatedByWeek(date("Y"), date("W"));
    $total_po = !empty($data['total_po']) ? $data['total_po'] : 0;
    $total_po_refund = !empty($data['total_po_refund']) ? $data['total_po_refund'] : 0;
    ...
    // This month
    $data = $this->dashboardManager->getPoCreatedByMonth(date("Y"), date("n"));
    $total_po = !empty($data['total_po']) ? $data['total_po'] : 0;
    $total_po_refund = !empty($data['total_po_refund']) ? $data['total_po_refund'] : 0;
