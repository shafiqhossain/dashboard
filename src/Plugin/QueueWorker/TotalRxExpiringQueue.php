...
/**
 * Defines 'custom_example_total_rx_expiring_queue' queue worker.
 *
 * @QueueWorker(
 *   id = "custom_example_total_rx_expiring_queue",
 *   title = @Translation("Total Rx Expiring Queue"),
 *   cron = {"time" = 60},
 * )
 */
class TotalRxExpiringQueue extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public function processItem($data) {
    $this->logger->info('custom_example_total_rx_expiring_worker run completed @ ' . date('Y-m-d H:i:s'));

    $data = $this->dashboardManager->getTotalExpiringRxByDate(date("Y-m-d"));
    ...
    $this->dashboardManager->setBillingCountByDate(
      [
        ...
        'rx_expiring' => $total_rx,
        'rx_expiring_nids' => $total_rx_nids,
        'rx_expiring_last_update' => date('Y-m-d H:i:s'),
      ],
      [
        'rx_expiring' => $total_rx,
        'rx_expiring_nids' => $total_rx_nids,
