...
/**
 * Defines 'custom_example_total_profile_expiring_cc_queue' queue worker.
 *
 * @QueueWorker(
 *   id = "custom_example_total_profile_expiring_cc_queue",
 *   title = @Translation("Total Credit Card Expiring Profiles Queue"),
 *   cron = {"time" = 60},
 * )
 */
class TotalProfileExpiringCcQueue extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public function processItem($data) {
    $this->logger->info('custom_example_total_profile_expiring_cc_worker run completed @ ' . date('Y-m-d H:i:s'));

    $data = $this->dashboardManager->getTotalExpiringProfileCcByDate(date("Y-m-d"));
    $total_profiles = !empty($data['total_profiles']) ? $data['total_profiles'] : 0;
    ...
    $this->dashboardManager->setBillingCountByDate(
      [
        ...
        'profile_cc_expiring' => $total_profiles,
        'profile_cc_expiring_ids' => $total_profiles_ids,
        'profile_cc_last_update' => date('Y-m-d H:i:s'),
      ],
      [
        'profile_cc_expiring' => $total_profiles,
        'profile_cc_expiring_ids' => $total_profiles_ids,
