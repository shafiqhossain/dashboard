...
/**
 * Defines 'custom_example_total_order_submitted_queue' queue worker.
 *
 * @QueueWorker(
 *   id = "custom_example_total_order_submitted_queue",
 *   title = @Translation("Total Order Submitted Queue"),
 *   cron = {"time" = 60},
 * )
 */
class TotalOrderSubmittedQueue extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public function processItem($data) {
    $this->logger->info('custom_example_total_order_submitted_worker run completed @ ' . date('Y-m-d H:i:s'));

    $data = $this->dashboardManager->getOrdersByDate(date("Y-m-d"));
    $total_orders = !empty($data['total_orders']) ? $data['total_orders'] : 0;
