...
/**
 * Defines 'custom_example_clinic_vs_staff_rx_refills_queue' queue worker.
 *
 * @QueueWorker(
 *   id = "custom_example_clinic_vs_staff_rx_refills_queue",
 *   title = @Translation("Clinic vs Staff Rx Refills Queue"),
 *   cron = {"time" = 60},
 * )
 */
class ClinicVsStaffRxRefillsQueue extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public function processItem($data) {
    $this->logger->info( 'custom_example_clinic_vs_staff_rx_refills_worker run completed @ ' . date('Y-m-d H:i:s'));

    /** Rx Refills by Clinic **/
    ...
    $this->dashboardManager->setBillingInfoByDate(
      [
        ...
        'clinic_rx_refill_last_update' => date('Y-m-d H:i:s'),
      ],
      [
        'clinic_scit_refill' => $clinic_scit_refill,
        'clinic_slit_refill' => $clinic_slit_refill,
    ...
    // This week
    $data = $this->dashboardManager->getClinicTotalRxRefillByRxTypeByWeek(date("Y"), date("W"));
    $clinic_scit_refill = !empty($data['scit_refill']) ? $data['scit_refill'] : 0;
    $clinic_slit_refill = !empty($data['slit_refill']) ? $data['slit_refill'] : 0;
    ...
    // This week
    $data = $this->dashboardManager->getStaffTotalRxRefillByRxTypeByWeek(date("Y"), date("W"));
    $staff_scit_refill = !empty($data['scit_refill']) ? $data['scit_refill'] : 0;
    $staff_slit_refill = !empty($data['slit_refill']) ? $data['slit_refill'] : 0;
