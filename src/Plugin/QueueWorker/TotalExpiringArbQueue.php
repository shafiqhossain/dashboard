...
/**
 * Defines 'custom_example_total_expiring_arb_queue' queue worker.
 *
 * @QueueWorker(
 *   id = "custom_example_total_expiring_arb_queue",
 *   title = @Translation("Total Expiring ARB Queue"),
 *   cron = {"time" = 60},
 * )
 */
class TotalExpiringArbQueue extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public function processItem($data) {
    $this->logger->info('custom_example_total_expiring_arb_worker run completed @ ' . date('Y-m-d H:i:s'));

    $data = $this->dashboardManager->getTotalExpiringArbByDate(date("Y-m-d"));
    ...
    $this->dashboardManager->setBillingCountByDate(
      [
        ...
        'arb_expiring' => $total_subscriptions,
        'arb_expiring_ids' => $total_subscriptions_ids,
        'arb_expiring_last_update' => date('Y-m-d H:i:s'),
      ],
      [
        'arb_expiring' => $total_subscriptions,
        'arb_expiring_ids' => $total_subscriptions_ids,
