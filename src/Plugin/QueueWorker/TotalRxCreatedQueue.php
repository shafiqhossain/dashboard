...
/**
 * Defines 'custom_example_total_rx_created_queue' queue worker.
 *
 * @QueueWorker(
 *   id = "custom_example_total_rx_created_queue",
 *   title = @Translation("Total Rx Created Queue"),
 *   cron = {"time" = 60},
 * )
 */
class TotalRxCreatedQueue extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public function processItem($data) {
    $this->logger->info('custom_example_total_rx_created_worker run completed @ ' . date('Y-m-d H:i:s'));
    ...
    // This week
    $data = $this->dashboardManager->getTotalRxCreatedByWeek(date("Y"), date("W"));
    $total_slit_created = !empty($data['total_slit_created']) ? $data['total_slit_created'] : 0;
    $total_scit_created = !empty($data['total_scit_created']) ? $data['total_scit_created'] : 0;
